	...
	<div class="row">
		<div class="widget-holder col-lg-6 col-md-12 col-sm-12 col-xs-12">
			<div class="widget-bg">
				...
				<h5 class="widget-title"><a href="{{route('micrositeList')}}?game={{$game->id}}"><strong>{{$game->name}} </strong>Microsites</a></h5>
				@if (App\General::page_access(Auth::user()->group_id, 'game', 'create'))
				<div class="widget-actions">
					<a class="create btn btn-sm btn-primary ml-1" href="javascript:void(0);" data-callback="dataTable" data-type="post" data-url="{{ url('games') }}">Create New Microsites</a>
				</div>
				@endif
...
{!! $dataTable->scripts() !!}
<script src="{{ url('assets/js/microsite.js?v=0.1') }}"></script>
@endpush
