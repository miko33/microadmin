@extends('layouts.app')
@section('widget-body')
<link rel="stylesheet" href="{{ url('assets/vendors/image-picker/image-picker.css') }}">
<style>
                                                ...
                                                @foreach($options as $key => $option)
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <div class="lightbox-gallery ml-3" data-toggle="lightbox-gallery" data-type="image" data-effect="fadeInRight">
                                                                <div class="lightbox">
                                                                    <span class="slider-url" title="Click me to preview." style="cursor: pointer;">{{$option->url}}</span>
                                                                </div>
                                                            </div>
                                                        </td>
                                                        <td>
                                                        ...
                                                        <td>
                                                            <div class="btn-group">
                                                                <button type="button" class="edit-slider btn btn-default" data-key="{{$key}}" data-url="{{$option->url}}" data-alt="{{$option->alt}}">
                                                                    <i class="feather feather-edit-2"></i>
                                                                </button>
                                                                <button id="delete" data-delete-type="swal" data-type="delete" data-url="{{route('deleteSliderImage', [$microsite->hostname, $key])}}" type="button" class="btn btn-default">
                                    ...
                                    <button class="btn btn-sm btn-color-scheme btn-xl px-4 h6 my-0 fs-16 fw-500" style="margin-bottom: 50px !important" id="sfm-button">Select From Media</button>
                                        ...
                                        <div class="col-md-4 d-flex mb-0">
                                            <div class="form-group ml-auto">
                                                ...
                                                <button class="btn btn-sm btn-success px-4 h6 my-0 fs-16 fw-500" id="uploadSlider" data-microsite="{{$microsite->hostname}}" style="display: none">Save</button>
                                            </div>
                                        </div>
                                        <div class="col-md-4 d-flex mb-0">
                                        <form id="imageupload" action="javascript:void(0)" encttype="multipart/form-data">
                                                ...
                                                <div class="form-group ml-auto">
                                                    ...
                                                    <input type="hidden" id="host" value="{{$microsite->hostname}}">
            ...
            <div class="modal-body">
                <select class="image-picker show-html" id="image-selector">
                    @foreach($microsite->images()->get() as $image)
                        <option data-img-src="{{ $image->url }}" data-img-class="image-picker-preview" data-img-alt="{{ $image->url }}" value="{{ $image->url }}">  {{ $image->url }}  </option>
                    @endforeach
                </select>
            </div>
...
<script>
    ...
    var selectSliderFromMedia = "{{route('selectSliderFromMedia', $microsite->hostname)}}";
    var getSlidersRoute = "{{route('getSliderImage', [$microsite->hostname])}}";
    var getOrderedSliderRoute = "{{route('getOrderedSlider', [$microsite->hostname])}}";
    var reorderSliderRoute = "{{route('reorderSlider', [$microsite->hostname])}}";
    var editSliderRoute = "{{route('editSlider', [$microsite->hostname])}}";
</script>
<script src="{{ url('assets/vendors/image-picker/image-picker.min.js?v=0.1') }}"></script>
