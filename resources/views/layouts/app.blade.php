...
<head>
    ...
    <link href="{{ url('assets/css/switch.css') }}" rel="stylesheet" type="text/css" />

    <title>{{ ucwords(str_replace('_', ' ', request()->segment(count(request()->segments())))) ?: 'Dashboard' }} - {{ config('app.name', 'Laravel') }}</title>
            ...
            <nav class="navbar">
                ...
                <div class="navbar-header">
                    <a href="./index.html" class="navbar-brand">
                        <img class="logo-expand" alt="" src="{{ url('assets/demo/logo-expand.png') }}" />
                        <img class="logo-collapse" alt="" src="{{ url('assets/demo/logo-collapse.png') }}" />
                        <!-- <p>BonVue</p> -->
                    </a>
                </div>
                <!-- /.navbar-header -->
                    ...
                    <li class="dropdown">
                        <a href="javascript:void(0);" class="dropdown-toggle ripple" data-toggle="dropdown">
                            <span class="avatar thumb-xs2">
                                <img src="{{ url('assets/demo/user-cards/4.jpg') }}" class="rounded-circle" alt="" />
                    ...
                    <div class="side-user">
                        <div class="col-sm-12 text-center p-0 clearfix">
                            ...
                            <div class="lh-14 mr-t-5">
                                <a href="javascript:void(0);" class="hide-menu mt-3 mb-0 side-user-heading fw-500">{{ Auth::user()->name }}</a>
                ...
                <main class="main-wrapper clearfix">
                    ...
                    <div class="row page-title clearfix">
                        <div class="page-title-left">
                            <h6 class="page-title-heading mr-0 mr-r-5">  {{ ucwords(str_replace('_', ' ', request()->segment(count(request()->segments())))) ?: 'Dashboard' }}</h6>
                            @if(!isset($microsite_breadcrumb))
                            <p class="page-title-description mr-0 d-none d-md-inline-block">{{ App\General::breadcrumb(request()->segment(count(request()->segments())) ?: '')->description }}</p>
                            @else
                            <p class="page-title-description mr-0 d-none d-md-inline-block">{{$microsite_breadcrumb}}</p>
                            @endif
                        </div>
                        <!-- /.page-title-left -->
                        <div class="page-title-right d-inline-flex">
                            ...
                            @for($i = 1; $i <= count(Request::segments()); $i++)
                                ...
                                <?php $link .= '/' . Request::segment($i); ?>
                                    <li class="breadcrumb-item"><a href="{{ $link }}">{{ ucwords(str_replace('_',' ',Request::segment($i))) }}</a></li>
                                @else
                                    <li class="breadcrumb-item active">{{ ucwords(str_replace('_',' ',Request::segment($i))) }}</li>
                                @endif
                            @endfor
                            </ol>
                        </div>
